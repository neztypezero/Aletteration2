...
static void NezOpenALInterruptionCallback(void* soundController, UInt32 interruptionState) {
	NezOpenAL* openalSoundController = (__bridge NezOpenAL*)soundController;
...
-(id)init {
	if ((self = [super init])) {
		...
		InitAudioSession(kAudioSessionCategory_AmbientSound, NezOpenALInterruptionCallback, (__bridge void *)(self), PREFERRED_SAMPLE_OUTPUT_RATE);
...
-(void)setListenerVelocity:(GLKVector3)velocity {
...
-(GLKVector3)getListenerVelocity {
	GLKVector3 velocity;
...
-(void)setListenerOrientation:(GLKVector3)orientation {
	alListenerfv(AL_ORIENTATION, &orientation.x);
	ALenum al_error = alGetError();
	if(AL_NO_ERROR != al_error) {
...
-(GLKVector3)getListenerOrientation {
	GLKVector3 orientation;
	alGetListenerfv(AL_ORIENTATION, &orientation.x);
	ALenum al_error = alGetError();
	if(AL_NO_ERROR != al_error) {
